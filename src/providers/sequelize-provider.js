...
class SequelizeProvider extends SettingProvider {
  ...
  async init(client) {
    ...
    for (const row of rows) {
      ...
      try {
        ...
        client.emit(
          "warn",
          `PostgreSQLProvider couldn't parse the settings stored for guild ${
            row.dataValues.guild
          }.`
        );
        continue;
      }
      ...
      const guild =
        row.dataValues.guild !== "0" ? row.dataValues.guild : "global";
  ...
  get(guild, key, defVal) {
    ...
    return settings
      ? typeof settings[key] !== "undefined" ? settings[key] : defVal
      : defVal;
  }
  ...
  updateOtherShards(key, val) {
    ...
    this.client.shard.broadcastEval(`
      ...
      } && this.provider && this.provider.settings) {
